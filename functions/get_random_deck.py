...
def random_deck(available_cards, feature_names, num_decks = 1, output_format = "indices") : 
    ...
    mappings = mappings_from_available(available_cards, feature_names)
    base = mappings["base"]; evos = mappings["evos"]; heroes = mappings["heroes"]; card_collisions = mappings["feature_collisions"]
    ...
    for deck in range(num_decks) : 
        ...
        for slot in range(num_slots) : 
            ...
            cards_to_search += [hero_card for hero_card in heroes if hero_card not in collisions] # adds only champions if base only
            ...
            while True : 
                ...
                if cards_in_deck[deck, slot] in base : 
                    ...
                    num_evos += 1 
                    break 
                elif cards_in_deck[deck, slot] in heroes and num_heroes < max_heroes : 
                    num_heroes += 1 
                    break 
