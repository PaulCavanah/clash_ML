...
from functions.load_data_from_parquet import stream_onehot, stream_decks
...
class DataStreamConfig() : 
    ...
    def __init__(self, buffer_size = 1_000_000, filters = None, levels = False, base_only = False, unique = False) : 
        ...
        self.unique = unique # if True, only unique decks are included from each buffer (an option that only applies for streaming decks)
...
def stream_train_model_decks(model, val_loader, save_state_path, train_config : TrainConfig, stream_config: DataStreamConfig, obscure_config: ObscureConfig = None, device = "cpu") : 
    # Same as stream_train_model except it's for training randomly generated decks vs. real decks
    ...
    while epoch <= train_config.max_epochs : 
        ...
        model.train()
        data_generator = stream_decks(buffer_size = stream_config.buffer_size, filters = stream_config.filters, levels = stream_config.levels, base_only = stream_config.base_only, unique = stream_config.unique)
        ...
        for buffer in range(1, stream_config.num_buffers + 1) :
            ...
            # Create train loader - half real decks, half randomly generated decks
            X_real, _, feature_names = next(data_generator)
            ...
            if obscure_config : 
                ...
                mask[np.random.random((N, )) > obscure_config.p_partial, :] = False # Rows are protected from this operation at frequency 1-p_partial 
                X[mask] = 0 
